...
`parameter_binding` carries a residual of its own, and it is declared here rather than left to be discovered: what the check establishes is that the value at that position is a data structure and therefore not the statement. What the sink does with the values inside it is the sink's business, and the check does not follow them into the callee — the same boundary every other form stops at. A guarded position holding anything but such a structure stays a violation however safe its neighbours are; a sink that takes bound values one per argument is declared by naming the statement position in `positions`.
